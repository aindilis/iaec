package FRDCSA::IAEC::PurePerl::TextAnalysis;
...
use System::CausalRelationExtraction;
use System::KNext;
use System::MontyLingua;
use System::OpinionFinder;
...
use Class::MethodMaker
  ...
  [
   ...
   qw / MySayer Codes Skip DontSkip MyDateExtract MyTagger MyNER
	...
	MyTempAgent MyLight2 MyKNext MyOpinionFinder
	MyCausalRelationExtraction MySentence IsCached
	AllResultsWereCached /
...
sub init {
  ...
  $self->Inits
	...
	$self->MyLinkipedia
      ...
      # },
      OpinionFinder => sub {
	$self->MyOpinionFinder
	  (System::OpinionFinder->new
	   ());
      },
      CausalRelationExtraction => sub {
	$self->MyCausalRelationExtraction
	  (System::CausalRelationExtraction->new
	   ());
      },
     });
  ...
  $self->Codes
	...
	$self->MyLinkipedia->Linkify(Query => $_[0]->{Text});
      ...
      OpinionFinder => sub {
	my $self = $UNIVERSAL::textanalysis;
	my $sentences = $self->MySentence->Sentences
	  (
	   Text => $_[0]->{Text},
	   Clean => 1,
	  );
	$self->MyOpinionFinder->ProcessSentences
	  (Sentences => $sentences);
      },
      CausalRelationExtraction => sub {
	my $self = $UNIVERSAL::textanalysis;
	...
	my $text = join("\n",@$sentences);
	$self->MyCausalRelationExtraction->Parse
	  (Text => $text);
      },
