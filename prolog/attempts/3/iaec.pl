...
precondition_holds(Input,PreconditionList,State) :-
	append(Input,Rest,NewInput),
	foreach(member(Fact,State),(
				    %% show([asserting,Fact]),
				    assert(Fact))),
	(   forall(member(Precondition,PreconditionList),(call((term_variables(Precondition,NewInput),show([precondition(Precondition)]),Precondition))))
...
fire(Input,State1,Candidates,State2) :-
	...
	findall(Do,
		(   member(Candidate,Candidates),
		    ...
		    (	(   Results \= []) ->
		    	findall(Item,foreach(member(Effect,EffectList),(p,term_variables(Effect, NewInput),p,Item = Effect)),Do) ;
		    	findall(Item,foreach(member(Effect,EffectList),(p,term_variables(Effect, NewInput),p,Item = neg(Effect))),Do))),
		StateDiff),
...
eval_holds(Input,EdgeFunctionList,State,Results) :-
	...
	findall(Result,
		(   member(EdgeFunction,EdgeFunctionList),
			...
			Term =.. [P|Numbers],
			append(Input,Rest,NewInput),
			term_variables(EdgeFunction,NewInput),
			call(EdgeFunction),
